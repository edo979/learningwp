<article id="post-<?php the_ID(); ?>" class="<?php hybrid_entry_class(); ?>">
	...
	<?php } else { ?>
		...
		<?php if ( get_the_title() ) { ?>
			...
			<header class="entry-header">
				<?php the_title( '<h2 class="entry-title"><a href="' . esc_url( hybrid_get_the_post_format_url() ) . '" title="' . the_title_attribute( array( 'echo' => false ) ) . '">', '<span class="meta-nav">&rarr;</span></a></h2>' ); ?>
			</header><!-- .entry-header -->
		...
		<footer class="entry-footer">
			<?php echo apply_atomic_shortcode( 'entry_meta', '<div class="entry-meta">' . __( '[post-format-link] published on [entry-published] [entry-permalink before="| "] [entry-comments-link before="| "] [entry-edit-link before="| "]', 'celebrate' ) . '</div>' ); ?>
		</footer><!-- .entry-footer -->
