...
<div class="loop-meta">
	...
	<?php } elseif ( is_search() ) { ?>
		...
		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the search results for "%s"', 'celebrate' ), esc_attr( get_search_query() ) ) ); ?>
		</div><!-- .loop-description -->
	...
	<?php } elseif ( is_day() || is_month() || is_year() ) { ?>
		...
		<?php
			if ( is_day() )
				$date = get_the_time( __( 'F d, Y', 'celebrate' ) );
			elseif ( is_month() )
				$date = get_the_time( __( 'F Y', 'celebrate' ) );
			elseif ( is_year() )
				$date = get_the_time( __( 'Y', 'celebrate' ) );
		?>

		<h1 class="loop-title"><?php echo $date; ?></h1>

		<div class="loop-description">
			<?php echo wpautop( sprintf( __( 'You are browsing the site archives for %s.', 'celebrate' ), $date ) ); ?>
		</div><!-- .loop-description -->

	<?php } elseif ( is_archive() ) { ?>

		<h1 class="loop-title"><?php _e( 'Archives', 'celebrate' ); ?></h1>

		<div class="loop-description">
			<?php echo wpautop( __( 'You are browsing the site archives.', 'celebrate' ) ); ?>
		</div><!-- .loop-description -->
